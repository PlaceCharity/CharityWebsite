...
<div class="w-full justify-center items-center flex">
    <div class="w-full md:max-w-[45vw] flex flex-col items-center m-2">
        <div class="tabs w-full flex-grow-0">
            <button class="tab tab-lifted tab-border-none tab-lg flex-1 font-patrickhand text-xl uppercase {tab === 'original' ? 'tab-active' : ''}" on:click={() => tab = 'original'}>Original</button>
            <button class="tab tab-lifted tab-border-none tab-lg flex-1 font-patrickhand text-xl uppercase {tab === 'pixelated' ? 'tab-active' : ''}" on:click={() => tab = 'pixelated'}>Pixelated</button>
        </div>
        <div class="bg-base-100 w-full flex flex-col items-center justify-center rounded-xl p-6 shadow-xl {tab === 'original' ? 'rounded-tl-none' : ''} {tab === 'pixelated' ? 'rounded-tr-none' : ''}">
            ...
            <button class="btn btn-outline w-full font-patrickhand normal-case text-xl m-2 {image === null || tab === 'original' ? 'hidden' : ''}" on:click={download}>
                Download&nbsp;
